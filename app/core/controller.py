...
from app.core.forms import CreateContact, CreateOrganisation, CreateProject
...
@login_required
def view_organisations_table():
    cols = Organisation.metadata.tables['organisation'].columns.keys()
    org = Organisation.query.limit(25).all()
    return render_template('core/organisations_table.html', organisations=org, cols=cols)

@core.context_processor
def quick_create():
    COform = CreateOrganisation(request.form)
    CCform = CreateContact(request.form)
    CPform = CreateProject(request.form)
    return dict(CCform=CCform,
                COform=COform,
                CPform=CPform)
